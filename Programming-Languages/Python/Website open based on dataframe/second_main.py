...
def open_online(filem: str, sleep_time: int or float, left_place=None):
    ...
    for order, link in enumerate(df_link):
        try:
            ...
            #     length = length - 1
            #     while (length != 0):
            #         window_nam = DRIVER.window_handles[length]
            #         length = length - 1
            #         DRIVER.switch_to.window(window_name=window_nam)
            #         DRIVER.close()
            print("error occured")
...
if __name__ == "__main__":
    ...
    done_files_dict = {"biten dosya isimler": list(DONE_FILES)}
    delay = config["gecikme"]
    if file_name:
        left_file_num_global = len(DONE_FILES)
        open_online(file_name, delay, left_place)
        done_files_dict["biten dosya isimler"].append(file_name)
    ...
    if FILES:
        for i, excel_file_name in enumerate(FILES):
            ...
            done_files_dict["biten dosya isimler"].append(excel_file_name)
            with open("bitmis_dosyalar.json", "w") as done_json:
                json.dump(done_files_dict, done_json)
