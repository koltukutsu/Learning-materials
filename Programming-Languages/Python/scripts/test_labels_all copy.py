...
image_folder_path = abspath(getcwd()) + "/"
label_folder_path = abspath(getcwd()) + "/"  # resimlerin bulundugu yolu veriyoruz burada.

ISTENILEN_RENK = (36,255,12)
...
# for f in filenames:
    ...
    image_name = image_folder_path + f + img_type
    ...
    if isfile(txt_name): # isfile bool donduruyor, try except e gerek yok
    ...
    for label in labels:
        lab = label[:-2] # son 2 elemana kadar niye aliyor
        values = lab.split(' ')
        try:
	        ...
	        # color = ISTENILEN_RENK # yukarida zaten atadik
	        img = cv2.rectangle(img, start_point, end_point, ISTENILEN_RENK, 3)
	        cv2.putText(img, klas, (start_point[0], start_point[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, ISTENILEN_RENK, 2)
        except:
        	print(f, values[0])

    cv2.putText(img, f, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, ISTENILEN_RENK, 3)
    img = cv2.resize(img, (1600,800), interpolation = cv2.INTER_AREA)                  # keyfinize gore piksel degerleri girebilirsiniz
    cv2.imshow(image_name.split("/")[-1], img)
    
    control = cv2.waitKey(1) & 0xFF
    
    if control == ord("d"): ## burada while kullanmak mantikli olur, istedigimiz yere atlariz
        try:
            tasinanlar_path = image_folder_path + "TASINANLAR/"
            mkdir(tasinanlar_path)
        except FileExistsError:
            pass
        
        sys.rename(image_name, tasinanlar_path + f + img_type)
        sys.rename(txt_name, tasinanlar_path + f + '.txt')
        index += 1
        
    elif control == ord("b"): 
        ## for dongusunde, geri donmemiz zor. Herseyi bu if'in altina yazmak gerekir
        # yeniden while ile yapilabilir
        # veyahut yukaridaki teferruati bir fonksiyon olarak yapabiliriz    
        
        ## index ayni kalacak, while kullandik
        continue
    elif control == ord("q"):
        print("PROGRAM KAPATILIYOR!!!")
        print("Kalinan Image: ", f + img_type)
        break
    else:
        index += 1
