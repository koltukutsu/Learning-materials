...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.KEY_WORDS = ['belgeyi_kontrol_et', 'PDF Kontrol Edilecek',
                          'PDF Hatasi', 'hata_kontrol_et', 'Bakilacak']
        self.index = 0
        self.root_for_excel = ""

        self.ui.pdf_goster.settings().setAttribute(
    ...
    def content_display(self):
        ...
        # dir_path den alinan yol \ seklindedir, benim de \'lari / ile degistirmem lazim
        eklenecek = self.root_for_excel.split("\\")[:-1]
        # suan elimde normal path var, sorun yok
        eklenecek = "/".join(eklenecek) + "/"
    ...
    def content_change(self):
        ...
        self.df_hatali[self.value_col][self.index] = self.ui.degerler.toPlainText(
        ).replace(" ", "").replace("\n", "")
    ...
    def save_and_exit(self):
        ...
        if self.ui.tercih.isChecked():
            self.df_exit = self.df_exit.append(
                self.df_hatali, ignore_index=True)
        else:
            self.df_exit = self.df_exit.append(
                self.df_hatali, ignore_index=True)
            self.df_exit = self.df_exit.append(
                self.df_temiz, ignore_index=True)

        print("to exit")
        self.saving_path = self.root_for_excel.rstrip(".xlsx") + "YENI.xlsx"
        self.df_exit[[self.id_col, self.value_col]].to_excel(
            self.saving_path, index=False)
        QtWidgets.QMessageBox.about(
            self, "Kaydedildi", f"Kaydedilen Dosya Yolu: {self.saving_path}")
        print("Finito")
